...
private const val KOTLINX_JSON_CLASS_DEFAULT = "kotlinx.serialization.json.Json.Default"
val jsonClasses = setOf(KOTLINX_JSON_CLASS, KOTLINX_JSON_CLASS_DEFAULT)
        ...
        override fun createUastHandler(context: JavaContext): UElementHandler? {
            return object : UElementHandler() {
                override fun visitCallExpression(node: UCallExpression) {
                    if (node.receiver?.getExpressionType()?.canonicalText?.let {
                            jsonClasses.contains(it)
                        } == true) {
